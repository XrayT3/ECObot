...
from Nurym import const, base, markups, files, temp, config
...
def celler_panel(call):
    bot.edit_message_text('Раздел покупателей отходов.',
...
def registraciy_panel(message):
    sent = bot.send_message(message.chat.id, "Введите данные компании (Название, БИН)",
                            reply_markup=markups.return_to_menu())
    bot.register_next_step_handler(sent, hello)


def hello(message):
    bot.send_message(211439710, 'Новая заявка на регистрацию. {name} '.format(name=message.text) + ' id= ' + str(message.from_user.id))
